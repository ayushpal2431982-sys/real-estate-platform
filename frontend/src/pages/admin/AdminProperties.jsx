...
const AdminProperties = () => {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm(
            "Are you sure want to delete this property? This action is permanent."
        )) return;

        try {                                            // ✅ Single try block (removed nested try)
            await axios.delete(`${API_URL}/api/admin/properties/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            setProperties(properties.filter((p) => p._id !== id));
        }
        catch (err) {                                   // ✅ Properly closed catch
            alert("Failed to delete property");
        }
    };                                                  // ✅ handleDelete properly closed

    if (loading)                                        // ✅ Moved outside handleDelete
        return (
        ...
        <>
            <div className={s.headerContainer}>
                ...
                <p className={s.pageSubtitle}>
                    Review and manage all property listing across the platform.
                </p>
            </div>
            <div className={s.headerContainer}>
                {properties.length === 0 ? (
