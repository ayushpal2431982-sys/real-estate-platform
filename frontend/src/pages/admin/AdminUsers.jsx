...
const AdminUsers = () => {
  ...
  const handleBlock = async (id) => {
    try {
      ...
      if (res.data.success) {
        // ✅ Fix 1: was `user.map` — should be `users.map`
        setUsers(
          users.map((u) =>
  ...
  const handleDelete = async (id) => {
    ...
    try {
      // ✅ Fix 2: was `/api/admin/users${id}` — missing `/` before id
      await axios.delete(`${API_URL}/api/admin/users/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      // ✅ Fix 3: was `user.filter` — should be `users.filter`
      ...
      <div className={s.cardContainer}>
        ...
        <div className={s.tableWrapper}>
          <table className={s.table}>
            ...
            <tbody>
              {/* ✅ Fix 4: was `filteredUsers.length > 0 (` — missing `?` ternary operator */}
