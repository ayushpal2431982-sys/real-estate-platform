...
const ChatMessages = () => {
  ...
  const [newMessages, setNewMessages] = useState(""); // ✅ single source of truth
    ...
    if (socket) {
      socket.on("receiveMessage", (data) => {
        if (activeChat && data.chatId === activeChat._id) {
          // ✅ Fix: Only add message if it's from the other person
          if ((data.sender?._id || data.sender) !== user._id) {
  ...
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (!newMessages.trim() || !activeChat) return; // ✅ Fix: was newMessage

    const textToSend = newMessages; // ✅ Fix: was newMessage
    setNewMessages(""); // ✅ Fix: was setNewMessage
    ...
    try {
      ...
      if (res.data.newMessage) {
        // ✅ Fix: Add message locally from API response only
        setMessages((prev) => [...prev, res.data.newMessage]);
        // ✅ Fix: Emit via socket for the OTHER person only
        sendMessage(activeChat._id, res.data.newMessage);
      }
      scrollToBottom();
    } catch (err) {
