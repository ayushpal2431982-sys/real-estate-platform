...
chatRouter.post("/send", async (req, res) => {
    try {
        ...
        const newMessage = {
            ...
            text,
            image,
            createdAt: new Date() // ✅ Fix 1: was new Data() (typo)
        };

        chat.messages.push(newMessage);
        await chat.save();

        const savedMessage = chat.messages[chat.messages.length - 1]; // ✅ Fix 2: was saveMessage (typo)
...
chatRouter.delete("/:chatId", async (req, res) => { // ✅ Fix 3: was "/chatId" (missing colon)
    try {
        const userId = req.user._id;
